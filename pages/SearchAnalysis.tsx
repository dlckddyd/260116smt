      ...
      datasets: [
        {
          ...
          backgroundColor: (context: any) => {
            ...
            gradient.addColorStop(0, 'rgba(0, 199, 60, 0)');
            gradient.addColorStop(1, 'rgba(0, 199, 60, 0.05)');
            return gradient;
          },
          fill: true,
          tension: 0.4,
          pointRadius: 0,
          pointHoverRadius: 6,
          pointHoverBackgroundColor: '#00c73c',
          ...
          <div className="animate-fade-in-up space-y-12">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center bg-gray-50 p-6 rounded-2xl border border-gray-100">
               <div className="flex items-center gap-4 mb-4 md:mb-0">
                   <h2 className="text-3xl font-bold text-gray-900"><span className="text-brand-accent">{data.mainKeyword.relKeyword}</span></h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4"><span className="text-gray-500 font-bold">월간 총 검색량</span><Search className="w-5 h-5 text-blue-500" /></div>
                    <div className="text-3xl font-bold text-gray-900 mb-2">{formatNumber(safeParseInt(data.mainKeyword.monthlyPcQc as string | number) + safeParseInt(data.mainKeyword.monthlyMobileQc as string | number))}</div>
                    <div className="flex gap-2 text-xs font-medium"><span className="bg-blue-50 text-blue-600 px-2 py-1 rounded">PC {formatNumber(data.mainKeyword.monthlyPcQc)}</span><span className="bg-green-50 text-green-600 px-2 py-1 rounded">Mo {formatNumber(data.mainKeyword.monthlyMobileQc)}</span></div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4"><span className="text-gray-500 font-bold">총 콘텐츠 발행량</span><FileText className="w-5 h-5 text-purple-500" /></div>
                    <div className="text-3xl font-bold text-gray-900 mb-2">{formatNumber((Object.values(data.content) as number[]).reduce((a, b) => a + b, 0))}</div>
                    <div className="flex gap-2 text-xs font-medium text-gray-400">블로그, 뉴스, 쇼핑 등 합계</div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4"><span className="text-gray-500 font-bold">콘텐츠 포화지수</span><PieChart className="w-5 h-5 text-red-500" /></div>
                    <div className="text-3xl font-bold text-gray-900 mb-2">{(() => { const totalSearch = safeParseInt(data.mainKeyword.monthlyPcQc as string | number) + safeParseInt(data.mainKeyword.monthlyMobileQc as string | number); const totalContent = (Object.values(data.content) as number[]).reduce((a, b) => a + b, 0); if (totalSearch === 0) return '-'; return (totalContent / totalSearch * 100).toFixed(1) + '%'; })()}</div>
                    <div className="text-xs text-gray-400">(총 발행량 / 총 검색량) * 100</div>
                </div>
                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                ...
                <div>
                    ...
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm overflow-x-auto">
                        <table className="w-full min-w-[600px]">
                            ...
                            <tbody>{data.dailyTrend.map((item, idx) => (<tr key={idx} className="border-b border-gray-100 hover:bg-gray-50 text-sm"><td className="py-4 px-6 text-center text-gray-500">{item.date}</td><td className="py-4 px-6 font-bold">{item.keyword}</td><td className="py-4 px-6 text-right">{formatNumber(item.pc)}</td><td className="py-4 px-6 text-right">{formatNumber(item.mobile)}</td><td className="py-4 px-6 text-right font-bold text-blue-600">{formatNumber(item.total)}</td></tr>))}</tbody>
                        </table>
                    </div>
                </div>
            )}
            
            <div className="flex justify-center text-gray-300"><ArrowDown className="w-6 h-6 animate-bounce" /></div>
